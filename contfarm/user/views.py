...
class SignUp(APIView):
    ...
    def post(self, request):
        ...
        if role not in ["farmer", "contractor"]:
            return Response({"error": "Role must be 'farmer' or 'contractor'."}, status=status.HTTP_400_BAD_REQUEST)
...
class ProfileView(APIView):
    ...
    def get(self, request, pk):
        ...
        if prof.role == "farmer":
            response_data.pop("gstin", None)
        elif prof.role == "contractor":
            response_data.pop("screenshot", None)
            response_data.pop("isValid", None)
