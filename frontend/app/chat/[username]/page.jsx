    ...
    return receiveMessages.map((msg, index) => {
      ...
      return (
        <div className="px-4" key={index}>
          {showDate && (
            <div className="text-center text-sm text-gray-500 font-medium my-2">{messageDate}</div>
          )}
          <div className={`flex ${msg.username === currentUser ? "justify-end" : "justify-start"}`}>
            <div className={`px-4 py-2 flex justify-between items-end rounded-lg shadow-md max-w-xs break-words ${
                msg.username === currentUser ? "bg-blue-500 text-white" : "bg-gray-300 text-black"
              }`}
            >
              <p className="text-base mb-2">{msg.message}</p>
              <p className="text-xs text-gray-400 ml-2">{formatTime(msg.timestamp)}</p>
            </div>
          </div>
        </div>
      );
        ...
        <div className="p-4 bg-white shadow-md">
          {currentChat ? (
            <div className="flex items-center space-x-3">
              <img src={currentChat.profile.image} alt="Avatar" className="w-10 h-10 rounded-full" />
            ...
            <div className="flex flex-col items-center justify-center h-[calc(100vh-10rem)] text-center text-gray-500">
              <img src="/chatIcon.png" alt="Chat Illustration" className="w-48 h-48 mb-4 opacity-80" />
        ...
        {currentChat && (
          <div className="fixed bottom-0 left-0 right-0 bg-white p-4 shadow-lg">
            <div className="max-w-2xl mx-auto flex gap-2">
              <Input value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Type a message..." className="flex-1 border-2 border-green-700" />
